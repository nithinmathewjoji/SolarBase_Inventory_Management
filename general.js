const COLLECTION_NAME = 'general_inventory';
...
const firebaseConfig = {
  ...
  async function addItem() {
    ...
    await db.collection('general_inventory').add(item);
    loadItems();
  }
  ...
  async function loadItems() {
    const snapshot = await db.collection('general_inventory').get();
  ...
  async function deleteItem(id) {
    await db.collection('general_inventory').doc(id).delete();
    loadItems();
  }
  ...
  async function updateItemQuantity(id) {
    ...
    await db.collection('general_inventory').doc(id).update({
      quantity: newQty
    });
    loadItems();
  }
